...
import { articleUrl, articles, categoryName, getArticle, getRelatedPublishedArticles } from "../../content";
...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  return <main><JsonLd data={schema} /><SiteHeader />
      <section className="article-hero"><div className="shell"><div className="breadcrumbs"><a href="/">ホーム</a><span>/</span><a href={`/${article.category}/`}>{categoryName(article.category)}</a><span>/</span><span>{article.title}</span></div><p className="article-category">{categoryName(article.category)}</p><h1>{article.title}</h1><p className="article-description">{article.description}</p><div className="article-meta">{article.status === "published" ? `資料確認：${article.updatedAt}` : "公開準備中"} 編集：おなら研究所 編集部</div></div></section>
    <section className="article-page-section"><div className="shell article-layout"><article className="article-content">
      {article.status === "planned" && <section className="article-conclusion planned-note"><p>公開準備中</p><h2>{article.description}</h2><span>正式記事化前の概要ページです。トップやカテゴリから迷わず着地できるよう、要点だけ先に表示しています。</span></section>}
      {article.conclusion && <section className="article-conclusion"><p>この記事の結論</p><h2>{article.conclusion}</h2></section>}
      ...
      {article.sources && article.sources.length > 0 && <section className="article-sources"><h2>参考資料</h2><ul>{article.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.title}</a><span>{source.publisher}</span></li>)}</ul></section>}
