  ...
  return <main>
    ...
    <section className="section food-section">
      ...
      <div className="shell topic-visual-grid"><figure className="topic-photo food-photo"><img src={visualAssets.food.src} alt={visualAssets.food.alt} loading="lazy" decoding="async" /><figcaption><span>FOOD NOTE</span><strong>食材ごとの疑問を、ひとつずつ。</strong></figcaption></figure>
        <div className="topic-detail"><p>食品名だけで原因を決めず、成分、食べ方、量、個人差を分けて考えます。</p><div className="chips">{foodTopics.map((food) => <a key={food} href={foodTopicLinks[food] ?? "/food/"}>{food}</a>)}</div></div>
      </div>
    </section>
    ...
    <section className="section wash-section">
      ...
      <div className="shell topic-visual-grid scene-visual-grid"><div className="topic-detail"><p>朝、食後、寝ている間など、気になるタイミングを入口にします。生活の中の条件を分けて、落ち着いて見ていきます。</p><div className="chips">{sceneTopics.map((scene) => <a key={scene} href={sceneTopicLinks[scene] ?? "/scene/"}>{scene}</a>)}</div></div>
    ...
    <section id="search-results" className="section wash-section search-results"><div className="shell"><SectionHeading kicker={submittedQuery ? `SEARCH: ${submittedQuery}` : "ARTICLE INDEX"} title={submittedQuery ? `「${submittedQuery}」の検索結果` : "記事一覧"} description={submittedQuery ? "サイト内の記事を対象に検索しています。" : "気になる記事を選んで、詳しく読めます。"} />
      {visibleArticles.length > 0 ? <div className="article-grid">{visibleArticles.map((article) => <ArticleCard key={article.slug} article={article} />)}</div> : <div className="empty-result"><span aria-hidden="true">⌕</span><p>一致する記事が見つかりませんでした。</p><a href="/">トップの記事一覧へ戻る</a></div>}
    </div></section>
